...
namespace QuizParseLibrary
{
    ...
    public class QuizParser
    {
        ...
        private Regex m_rxOrderingFirstLine;
        private Regex m_rxTextFirstLine;
        private Regex m_rxImageFirstLine;
        ...
        public QuizParser()
        {
            ...
            //First line of ordering
            m_rxOrderingFirstLine = new Regex(@"^\s*order (?<" + QUESTION_NUMBER + @">\d+)[\)\.]\s*(?<" + QUESTION_TEXT + @">.+)$", RegexOptions.IgnoreCase);

            //First line of text information
            m_rxTextFirstLine = new Regex(@"^\s*text (?<" + QUESTION_NUMBER + @">\d+)[\)\.]\s*(?<" + QUESTION_TEXT + @">.+)$", RegexOptions.IgnoreCase);

            //First line of image information
            m_rxImageFirstLine = new Regex(@"^\s*image (?<" + QUESTION_NUMBER + @">\d+)[\)\.]\s*(?<" + QUESTION_TEXT + @">.+)$", RegexOptions.IgnoreCase);
        ...
        private Question ParseQuestion(string strQuestion)
        {
            ...
            Match matchOrderingFirstLine = m_rxOrderingFirstLine.Match(strLines[0]);
            Match matchTextFirstLine = m_rxTextFirstLine.Match(strLines[0]);
            Match matchImageFirstLine = m_rxImageFirstLine.Match(strLines[0]);
            ...
            // it's an ordering question, to be used by brightspace only
            else if (matchOrderingFirstLine.Success)
            {
                //End the legacy question block - we've found a modern format question
                m_legacyParser.ClearActiveQuestionBlock();

                //Get the question values from the expression
                string strQuestionNumber = matchOrderingFirstLine.Groups[QUESTION_NUMBER].Value.Trim();

                question.QuestionText = matchOrderingFirstLine.Groups[QUESTION_TEXT].Value.Trim();
                question.QuestionNumber = int.Parse(strQuestionNumber);
                ...
                for (int i = 1; i < strLines.Length; ++i)
                {
                    //Add the answer to the question
                    Answer answer = new Answer();
                    answer.AnswerID = i.ToString();
                    answer.IsCorrect = true;
                    answer.Text = strLines[i];

                    question.AddAnswer(answer);
                }
                question.Type = QuestionType.QT_ORDERING;
            }

            // it's a matching question, to be used by brightspace only
            else if (matchMatchFirstLine.Success)
            {
                ...
                //Get the question values from the expression
                string strQuestionNumber = matchMatchFirstLine.Groups[QUESTION_NUMBER].Value.Trim();

                question.QuestionText = matchMatchFirstLine.Groups[QUESTION_TEXT].Value.Trim();
                question.QuestionNumber = int.Parse(strQuestionNumber);
            ...
            else if (matchTextFirstLine.Success)
            {
                //End the legacy question block - we've found a modern format question
                m_legacyParser.ClearActiveQuestionBlock();

                //Get the question values from the expression
                string strQuestionNumber = matchTextFirstLine.Groups[QUESTION_NUMBER].Value.Trim();

                question.QuestionText = matchTextFirstLine.Groups[QUESTION_TEXT].Value.Trim();
                question.QuestionNumber = int.Parse(strQuestionNumber);

                question.Type = QuestionType.QT_TEXT;

            }

            else if (matchImageFirstLine.Success)
            {
                //End the legacy question block - we've found a modern format question
                m_legacyParser.ClearActiveQuestionBlock();

                //Get the question values from the expression
                string strQuestionNumber = matchImageFirstLine.Groups[QUESTION_NUMBER].Value.Trim();

                question.QuestionText = matchImageFirstLine.Groups[QUESTION_TEXT].Value.Trim();
                question.QuestionNumber = int.Parse(strQuestionNumber);
                ...
                question.Type = QuestionType.QT_IMAGE;
            }
