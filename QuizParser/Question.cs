        ...
        public string GetBrightspaceTestQuestionString()
        {
            ...
            switch (m_questionType)
            {
                ...
                case QuestionType.QT_TRUE_OR_FALSE:
                    ...
                    break;
                case QuestionType.QT_MATCHING:
                    strQuestion = "NewQuestion,M,\n";
                    strQuestion = strQuestion + "QuestionText," + m_strQuestionText + ",\n";
                    int count = 0;
                    foreach (Answer answer in m_lstAnswers)
                    {
                        count++;
                        var result = answer.Text.Split('/');
                        if (result[0].ToString().Trim() != string.Empty)
                            strQuestion += "Choice," + count + "," + result[0].ToString().Trim() + ",\n";
                        if (result[1].ToString().Trim() != string.Empty)
                            strQuestion += "Match," + count + "," + result[1].ToString().Trim() + ",\n";
                    }
                    break;
                case QuestionType.QT_ORDERING:
                    strQuestion = "NewQuestion,O,\n";
                    strQuestion = strQuestion + "QuestionText," + m_strQuestionText + ",\n";
                    foreach (Answer answer in m_lstAnswers)
                    {
                        if (answer.Text.Trim() != string.Empty)
                            strQuestion += "Item," + answer.Text.ToString().Trim() + ",\n";
                    }
                    break;
                case QuestionType.QT_TEXT:
                    strQuestion = "NewQuestion,TXT,\n";
                    strQuestion = strQuestion + "QuestionText," + m_strQuestionText + ",\n";
                    break;
                case QuestionType.QT_IMAGE:
                    strQuestion = "NewQuestion,IMG,\n";
                    string imageLocation = m_lstAnswers[0].Text.Split('=')[1];
                    string imageCaption = m_lstAnswers[1].Text.Split('=')[1];
                    strQuestion = strQuestion + "Image," + imageLocation + "\n";
                    strQuestion = strQuestion + "Caption," + imageCaption;
                    break;
                case QuestionType.QT_UNKNOWN:
                    return "ERROR";
            }
