...
public partial class QuizGenerator : System.Web.UI.Page
{
    ...
    private void FillQuestionTypeDropDownList()
    {
        ...
        this.lstSampleQuestionTypes.Items.Add(new ListItem("Matching", ((int)QuestionType.QT_MATCHING).ToString()));
        this.lstSampleQuestionTypes.Items.Add(new ListItem("Ordering", ((int)QuestionType.QT_ORDERING).ToString()));
        this.lstSampleQuestionTypes.Items.Add(new ListItem("Text (Information)", ((int)QuestionType.QT_TEXT).ToString()));
        this.lstSampleQuestionTypes.Items.Add(new ListItem("Image (Information)", ((int)QuestionType.QT_IMAGE).ToString()));

        //Note: Matching not supported for now
        /*
        QT_UNKNOWN = -1,
        QT_MULTIPLE_CHOICE,
        QT_MULTIPLE_ANSWER,
    ...
    private byte[] GetTestQuestionBytesBrightspace(Quiz quiz)
    {
        UTF8Encoding encoding = new System.Text.UTF8Encoding();

        string strQuestions = quiz.GetBrightspaceTestQuestionString();
        byte[] bytes = encoding.GetBytes(strQuestions);
    ...
    protected void OnDownloadBrightspace(object sender, EventArgs e)
    {
        Quiz quiz = this.Session["quiz"] as Quiz;
        ...
        string strFileName = StringUtils.GetFileSafeVersion(quiz.QuizName); //Note: IIS might actually do this automagically
        if (strFileName.Length == 0)
            strFileName = "brightspaceQuiz";

        //Write the zip file to the response object
        Response.Clear();
        Response.ContentType = "text/plain";
        Response.AddHeader("content-disposition", "attachment;filename=" + strFileName + ".csv");
        byte[] fileBytes = this.GetTestQuestionBytesBrightspace(quiz);
        Response.Write('\uFEFF'); //Specify the Byte Order Mark (BOM) So that special characters are interpretted correctly
        Response.OutputStream.Write(fileBytes, 0, fileBytes.Length);
        Response.Flush();
        ...
        switch (type)
        {
            ...
            case QuestionType.QT_ORDERING:
                strToAdd +=
                    ...
                    "Neptune";
                break;
            case QuestionType.QT_TEXT:
                strToAdd +=
                    "text 1. Please refer to the following passage to answer the next 10 questions \n";
                break;
            case QuestionType.QT_IMAGE:
                strToAdd +=
                    "image 1. Refer to the following graph to answer the next three questions. \n" +
                    "img=quizimages/populationgraph.png \n" +
                    "imgcap=Graph depicting world population growth during the last ten years.";
                break;
